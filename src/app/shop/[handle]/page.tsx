...
export default function ProductPage() {
  ...
  useEffect(() => {
    async function loadProduct() {
      try {
        ...
        // Try to get product from Shopify API
        const productData = await getProduct(productHandle);
        console.log(productData);
        if (productData) {
          setProduct(productData);
          setInStock(productData.availableForSale);
          setCurrentPrice(productData.price);
          setCurrentCurrencyCode(productData.currencyCode);
  ...
  const handleAddToBag = () => {
    ...
    console.log("Added to bag:", {
      title: product?.title,
      variant: variantInfo?.title || "Default",
      quantity,
      price: currentPrice,
